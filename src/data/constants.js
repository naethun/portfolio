export const Bio = {
  name: "Nathan",
  roles: [
    "Software Engineer",
    "Project Manager",
    "Coding Instructor",
  ],
  description: "While being a full-time student studying Computer Science & Engineering.",
  github: "https://github.com/naethun",
  resume:
    "hhttps://drive.google.com/file/d/1Y2hwj59QZVRmYVfv0rEKobR-mcl5V7tE/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/naethun/",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Tailwind",
        image:
          "https://static-00.iconduck.com/assets.00/tailwind-css-icon-1024x615-fdeis5r1.png",
      }
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "https://ajeetchaulagain.com/static/7cb4af597964b0911fe71cb2f8148d64/87351/express-js.png",
      },
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "Java",
        image:
          "https://education.oracle.com/file/general/p-80-java.png",
      },
      {
        name: "C#",
        image:
          "https://static-00.iconduck.com/assets.00/c-sharp-c-icon-456x512-9sej0lrz.png",
      },
      {
        name: "C",
        image:
          "https://cdn.iconscout.com/icon/free/png-256/free-c-58-1175247.png",
      },
    ],
  },
  {
    title: "Other Technologies",
    skills: [
      {
        name: "Git",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Git_icon.svg/2048px-Git_icon.svg.png",
      },
      {
        name: "GitHub",
        image:
          "https://cdn1.iconfinder.com/data/icons/smallicons-logotypes/32/github-512.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Figma",
        image:
          "https://static-00.iconduck.com/assets.00/apps-figma-icon-2048x2048-ctjj5ab7.png",
      },
      {
        name: "Shell, Bash, Terminal",
        image:
          "https://www.svgrepo.com/show/376359/bash.svg",
      },
      {
        name: "Linux",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f1/Icons8_flat_linux.svg/1200px-Icons8_flat_linux.svg.png",
      },
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://pbs.twimg.com/profile_images/1549089349119139843/AuqvKpSs_400x400.png",
    role: "Software Engineer",
    company: "LightningATC",
    date: "May 2022 - Jun 2023",
    desc: "Engineered a Command-Line Interface software & Chrome Extension in TypeScript & Node.js, which automated tasks for users.",
    skills: [
      "Reverse Engineering",
      "ReactJS",
      "NodeJS",
      "HTML",
      "CSS",
      "JavaScript",
    ],
  },
  {
    id: 1,
    img: "https://yt3.googleusercontent.com/ytc/AIdro_nqJRBE-PvehwgWqiwAhPzjuDAi6kca33Z5mvp4UADlcyI=s900-c-k-c0x00ffffff-no-rj",
    role: "Lead Coding Instructor",
    company: "Code Ninjas",
    date: "Dec 2022 - Present",
    desc: "Organized business analytics and developed programs to optimize operations using Node.js and Python while continuing to educate our students.",
    skills: [
      "JavaScript",
      "NodeJS",
      "C#",
      "Python",
    ],
  },
  {
    id: 2,
    img: "https://img1.wsimg.com/isteam/ip/7230bf8c-78a3-4764-bbc6-b984344c2a04/Viet_Voices_Logo_Phrase.png/:/cr=t:0%25,l:0%25,w:100%25,h:100%25/rs=w:400,cg:true",
    role: "Data Analyst",
    company: "Viet Voices",
    date: "Jan 2024 - Jun 2024",
    desc: "Processed large data sets & visualized them to educate the Vietnamese community about our city.",
  },
  {
    id: 2,
    img: "https://img1.wsimg.com/isteam/ip/7230bf8c-78a3-4764-bbc6-b984344c2a04/Viet_Voices_Logo_Phrase.png/:/cr=t:0%25,l:0%25,w:100%25,h:100%25/rs=w:400,cg:true",
    role: "Project Manager & Field Associate",
    company: "Viet Voices",
    date: "Jan 2024 - Present",
    desc: "Learn a vast range of technologies, create documentation and guides for the team, and train new interns while developing project plans and complete the projects on time.",
  },
];

export const education = [
  {
    id: 0,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQQLZWQOYPjTdus9VobVJyBjjPdyc0tOGwOpA&s",
    school: "Southwestern Community College",
    date: "Aug 2022 - Present",
    grade: "3.6 GPA",
    desc: "Currently doing the pre-requesites for the following majors: Computer Science, Computer Engineering, Data Science, and Cognitive Science with a Specialization in Machine Learning and Neural Computation. My Bachelor's of Science degree at a university will depend on the major I transfer as.",
    degree: "Associate Degree for Transfer, Computer Science",
  },
  {
    id: 1,
    img: "https://i.pinimg.com/originals/fd/fd/bb/fdfdbb35dc080581686e32a94d67c7ad.jpg",
    school: "Transferring to a 4-year university",
    date: "Aug 2025 - June 2026",
    grade: "TBA",
    desc: "My top three schools to transfer to are UCSD, UCI, or SDSU. I plan on majoring in any of the following I have listed prior.",
    degree: "Bachelor's of Science, TBA",
  },
];

export const projects = [
  {
    id: 9,
    title: "Lightning CLI (Extended from LightningATC)",
    date: "July 2022 - Jan 2023",
    description:
      `A software that automated tasks for our users on Discord and across the NFT space. I focused on the backend modules, specifically the NFT ones. We built a community of 2,600+ members throughout the whole process.` +

      `Some features that we had: 
      MagicEden NFT Sniper, user set a price for a specific NFT, we handled searching the backend for it, and proccessed the transaction on the blockchain it as soon as it finds the matching parameters.
      Multi-Threaded CandyMachine NFT Minter, user set a NFT mint they wanted, we processed the transaction within a second as soon as it went live.`,
    image:
      "https://private-user-images.githubusercontent.com/76854326/348047720-a19a4c19-416a-425c-aead-4970afef175f.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..W-cye72Z2lLADXlPM5-Sf5UnQOGLiU2tz6Qzah6F7iI",
    tags: [
      "JavaScript",
      "TypeScript",
      "Node Js",
      "SOL & ETH Blockchain",
      "Rest API's",
    ],
    category: "CLI Software",
    github: "https://github.com/naethun/LightningCLI",
  },
  {
    id: 1,
    title: "Telios AIO",
    date: "Dec 2022 - April 2023",
    description:
      `Full-stack development by me and my colleague. I focused on the backend. The purpose of this chrome extension was to help their users checkout a desired product in order to make a profit. By the use of automation, requests, and web scraping— this chrome extension was able to do what a human could do within in seconds.

      During the peak of it's era, it was highly functional and successful.

      Currently, I am not sure what modules work but I'm sure the shopify autofill may not be depreciated.`,
    image:
      "https://private-user-images.githubusercontent.com/76854326/348052074-56c08831-dc94-4dcf-bd24-2715cf26f01e.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..WolY6kb2Iuw8vvqE4Mj4XqKdLCdlYDJ7nwd_KboB2q0",
    tags: ["Google Chrome Extension", "JavaScript", "HTML", "CSS", "jQuery"],
    category: "Chrome Extension",
    github: "https://github.com/naethun/Telios-AIO/",
  },
  {
    id: 2,
    title: "Software Engineering Club Website",
    date: "March 2023 - December 2023",
    description:
      `Organized and gave tasks to club members to create a cohesive flow for the project. I also was one of three people who approved commits from dev branches into main, and tutored most of our club members throughout this whole project.`,
    image:
      "https://i.postimg.cc/7brk0TZf/Screenshot-2024-07-12-at-12-59-34-AM.png",
    tags: ["Next JS", "JavaScript", "TypeScript", "HTML", "CSS"],
    category: "Website",
    github: "https://github.com/SWC-Software-Engineering-Club/SWE-Club-Website",
  },
];
